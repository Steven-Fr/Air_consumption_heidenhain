...
#---    -------------------------------
        if datatabella01 == now:                                                                                  #l'ora presunta coincide con l'ora della tabella scrivi normalmente
        ...
        elif datatabella01 < now:                                                                 #se l'ora della riga � minore di quella aspettata la macchina � stata spenta per piu di un ora e
            ...
            myTableDiag01.attachToCell(gtk.Label(txt((now))),       col=(bar01+1),row=3, xpadding=5)
            now = now - 1
        elif datatabella01 > now:            #se maggiore c'� un anomalia nella tabella ignora il valore perch� � stata manomessa manualmente e passo alla riga sucessiva
            Start01 = Start01 -1
    else:
...
while wline1 < 31:      #finche non ho scritto 31 colonne cicla
    ...
    if Start1 > 0:
        ...
        datatabella = datapartenza + datetime.timedelta(days=giornitab)                                 #calcolo il giorno in cui � stato scritto il valore in tabella
...
#----------------------------------
        ...
        elif datatabella < yesterday:     #se la data della riga � minore di quella aspettata la macchina � stata spenta per piu di un giorno e
